      ...
      >
            ...
            <div className="rounded-xl overflow-hidden border border-slate-800 dark:border-white/[0.06] bg-black/50">
              <div className="h-6 px-3 bg-white/[0.02] border-b border-white/[0.04] flex items-center justify-between text-[10px] text-slate-400 font-mono">
                <span className="flex items-center gap-1.5">
                  ...
                  <span className="text-accent-400/90 font-bold tracking-wider">SEGMENT LOG STREAM / 最新切片抓取日志</span>
                </span>
                <span className="text-accent-300 font-mono font-bold">{segmentLogs.length} 条</span>
              </div>
              <div className="p-2.5 font-mono text-[10px] leading-relaxed text-slate-400 max-h-32 overflow-y-auto space-y-0.5 select-all">
                {segmentLogs.map((log, idx) => (
                  <div key={idx} className="truncate">
                    ...
                    <span className={log.includes("%") ? "text-accent-300 font-medium" : ""}>{log}</span>
                  </div>
                ))}
              </div>
            </div>
        ...
        <div className="h-12 px-5 border-t border-slate-100 dark:border-white/[0.06] bg-slate-50/70 dark:bg-white/[0.015] flex items-center justify-between text-xs shrink-0">
          ...
          <button
            type="button"
            onClick={onClose}
            className="px-4 py-1.5 rounded-xl bg-accent-500 hover:bg-accent-600 text-xs font-semibold text-white transition cursor-pointer shadow-sm"
          >
            完成
          </button>
        </div>
